...
export function TrainingModal() {
  ...
  const [sendingDatasets, setSendingDatasets] = useState<Set<string>>(
    new Set()
  );
  ...
  const handleSendAll = async () => {
    ...
    try {
      const results = await Promise.allSettled(
        datasets.map((dataset) => sendToIndexer(dataset))
      );
    ...
    <Dialog open={showModal} onOpenChange={toggleModal}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Record workflow editing sessions to create training datasets for the
            copilot
          </DialogDescription>
        </DialogHeader>
                            ...
                            <div>
                              ...
                              <div className="max-h-32 overflow-y-auto rounded border bg-muted/50 p-2">
                                <ul className="space-y-1 font-mono text-xs">
                                  {formatEditSequence(dataset.editSequence).map(
                                    (desc, i) => (
                                      <li key={i}>{desc}</li>
                                    )
                                  )}
