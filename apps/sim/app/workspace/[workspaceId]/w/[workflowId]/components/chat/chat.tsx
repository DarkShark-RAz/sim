...
export function Chat() {
  ...
  const triggerWorkflowUpdate = useWorkflowStore(
    (state) => state.triggerUpdate
  );
  ...
  const handleWorkflowResponse = useCallback(
    (result: unknown) => {
      ...
      if (
        ...
        selectedOutputs
          .map((outputId) =>
            extractOutputFromLogs(result.logs as BlockLog[], outputId)
          )
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    try {
      ...
      const messageContent =
        sentMessage ||
        (chatFiles.length > 0 ? `Uploaded ${chatFiles.length} file(s)` : "");
      addMessage({
  ...
  const handleConfigureStartInputs = useCallback(() => {
    if (!activeWorkflowId || !startBlockId) {
      logger.warn(
        "Cannot configure start inputs: missing active workflow ID or start block ID"
      );
      return;
    }

    try {
      const normalizedExisting = normalizeInputFormatValue(
        startBlockInputFormat
      );

      const newReservedFields: StartInputFormatField[] =
        missingStartReservedFields.map((fieldName) => {
      ...
      const updatedFields: StartInputFormatField[] = [
        ...newReservedFields,
        ...normalizedExisting,
      ];
    ...
    >
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <div
          className="flex-none"
          onDragEnter={
            !activeWorkflowId || isExecuting ? undefined : handleDragEnter
          }
          onDragOver={
            !activeWorkflowId || isExecuting ? undefined : handleDragOver
          }
          onDragLeave={
            !activeWorkflowId || isExecuting ? undefined : handleDragLeave
          }
                  ...
                  <div className="flex-1">
                    ...
                    <div className="space-y-1">
                      {uploadErrors.map((err, idx) => (
                        <div
                          key={idx}
                          className="text-[10px] text-[var(--text-error)]"
                        >
                          {err}
                        </div>
                      ))}
          ...
          >
            ...
            <div className="relative">
              ...
              <div className="-translate-y-1/2 absolute top-1/2 right-[2px] flex items-center gap-[10px]">
                <Badge
                  onClick={() =>
                    document.getElementById("floating-chat-file-input")?.click()
                  }
                  title="Attach file"
                  className={cn(
